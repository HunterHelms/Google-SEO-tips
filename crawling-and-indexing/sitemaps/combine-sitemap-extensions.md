...
For each sitemap extension that you want to use in a sitemap you need to specify the
 respective namespace that declares the tags the extension supports. This is done with the `xmlns` attribute of the `urlset` tag. The namespaces for the sitemap
...
The order of the extension in the sitemap is irrelevant after the `<loc>` tag. Keep in mind the [general sitemap best practices](https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap#general-guidelines),
