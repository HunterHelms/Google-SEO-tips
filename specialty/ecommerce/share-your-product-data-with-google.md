...
To increase eligibility for [richer appearances and on more surfaces across Google](https://developers.google.com/search/docs/specialty/ecommerce/where-ecommerce-data-can-appear-on-google) that can bring more relevant traffic to your site, share your ecommerce product data with
...
- **Include structured data in your site's product pages** .
 Learn more about the benefits of adding structured data to your web pages .
- **Tell Google directly which products you want to show on Google** by uploading a feed to [Google Merchant Center](https://support.google.com/merchants/answer/188924) .
...
Google may at times use other approaches to extract data from pages. If you want to explicitly
 tell Google not to use content on a page to form a snippet, add a [`data-nosnippet` attribute](https://developers.google.com/search/docs/crawling-indexing/robots-meta-tag#data-nosnippet-attr) to that HTML element.
...
>
> Product data describes various attributes of
>  products, such as its title, description, color, pricing, and availability.

For smaller sites that are updated less frequently, you can use an [automated feed to build your product data](https://support.google.com/merchants/answer/7538732) from crawled web content (structured data can help improve the accuracy of data extraction).
 This approach can also be useful to get started with less effort.

For larger sites or sites with frequently changing content, periodically [upload new data feed files](https://support.google.com/merchants/answer/188477) to Google Merchant Center (or for immediate updates use the [Content API](https://developers.google.com/shopping-content/guides/quickstart)).
...
To avoid this potential conflict in pricing and stock availability data (a common cause of
 synchronization issues), tell Google Merchant Center [to automatically update its copy of your product data](https://support.google.com/merchants/answer/3246284) based on the website contents, when such a discrepancy is noticed.
