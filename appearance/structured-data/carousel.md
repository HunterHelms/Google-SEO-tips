...
![An illustration of how a movie host carousel can appear in Google Search. It shows 3 different movies from the same website in a carousel format that users can explore and select a specific movie](https://developers.google.com/static/search/docs/images/movie-rich-result.png)

> [!NOTE]
> **Note**
>
> : There are other carousel-like features on Google
...
7. To keep Google informed of future changes, we recommend that you [submit a sitemap](https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap) . You can automate this with the [Search Console Sitemap API](https://developers.google.com/webmaster-tools/search-console-api-original/v3/sitemaps) .
...
The summary page defines an `ItemList`, where each `ListItem` has
 only three properties: `@type` (set to `ListItem`), `position` (the position in the list), and `url` (the URL of a page with
...
The details page defines the specific structured data type that the carousel is about. For example,
 if the summary page is about the best cookie recipes, each detail page would contain `Recipe` structured data for a specific recipe.
...
<html>
 ...
 "recipeIngredient": ["2 cups of peanut butter",
 "1/3 cup of sugar"],
 "recipeInstructions": [{
 "@type": "HowToStep",
...
<html>
 ...
 "recipeIngredient": ["2 cups of melted chocolate",
 "1/3 cup of sugar"],
 "recipeInstructions": [{
 "@type": "HowToStep",
...
<html>
 ...
 "recipeIngredient": ["2 cups of cinnamon",
 "1/3 cup of sugar"],
 "recipeInstructions": [{
 "@type": "HowToStep",
...
For your page to be eligible for a carousel rich result, you must follow the [Search Essentials](https://developers.google.com/search/docs/essentials) and [general structured data guidelines](https://developers.google.com/search/docs/guides/sd-policies).
...
1. Validate your code using the [Rich Results Test](https://search.google.com/test/rich-results) .
 ...
 type that the list is about: [Recipe](https://developers.google.com/search/docs/appearance/structured-data/recipe) , [Course](https://developers.google.com/search/docs/appearance/structured-data/course) , [Restaurant](https://developers.google.com/search/docs/appearance/structured-data/local-business#carousel-example) , [Movie](https://developers.google.com/search/docs/appearance/structured-data/movie) .
2. Deploy a few pages that include your structured data and use the [URL Inspection tool](https://support.google.com/webmasters/answer/9012289) to test how Google sees the page. Be sure that your page is
 accessible to Google and not blocked by a robots.txt file, the `noindex` tag, or
...
> : Allow time for re-crawling and re-indexing. Remember that it
>  may take several days after publishing a page for Google to find and crawl it.
3. To keep Google informed of future changes, we recommend that you [submit a sitemap](https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap) . You can automate this with the [Search Console Sitemap API](https://developers.google.com/webmaster-tools/search-console-api-original/v3/sitemaps) .
...
`ItemList` is the container item that holds all elements in the list. If used on a summary page, all URLs in the list must point to different pages on the same domain. If used on
...
| url | URL The canonical URL of the item detail page. All URLs in the list must be unique, but live
...
| Required properties |  |
| --- | --- |
| item | Thing An individual thing in a list. Populate this object with the following values, plus all the
 properties of the specific structured data type being described: item.name item.url Any other properties required for this data type, as described in schema.org and the rules
 described in the documents for your content type: Course Movie Recipe Restaurant Example : For a recipe, you would provide prepTime and image properties. |
| item.name | Text The string name of the item. The item.name is displayed as the title of an
 individual item in the carousel. HTML formatting is ignored. |
| item.url | URL The fully-qualified URL and page anchor to this item on the page. The URL must be
 the current page, and you must include an HTML anchor ( <a> tag or name or id value) in your page near the user-visible text. Example : https://example.org/recipes/pies#apple_pie . |
| position | Integer The item's position in the carousel. This is a 1-based number. |
