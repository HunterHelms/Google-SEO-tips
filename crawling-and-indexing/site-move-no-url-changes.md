...
1. **Prepare the new hosting infrastructure** .
 Upload your content to the new servers or configure your CDN and your origin servers, and
 test it.
2. **Start the site move** . Change the DNS settings of your
...
3. **Monitor traffic** . Keep tabs on the traffic served by the old
 and new hosting.
4. **Shut down** . Shut down the old hosting
...
- **Allow for public testing** with a temporary hostname for your new infrastructure (like `beta.example.com` ) so you can test accessibility by browsers. A temporary
 hostname can help you test whether Googlebot can reach your site or not. To prevent
 accidentally letting the test site get indexed, add the [`noindex` `robots` rule](https://developers.google.com/search/docs/crawling-indexing/block-indexing) to the HTML or the HTTP headers of your pages.

### Check that Googlebot is able to access the new hosting infrastructure

If you don't already have a Search Console account, [create a new account](https://support.google.com/webmasters/answer/34592) for your site to help you monitor Google access and traffic. If you created a temporary
 hostname for your new site, verify that property as well. Check that Googlebot can access your
 new infrastructure using the [URL Inspection Tool](https://support.google.com/webmasters/answer/9012289) in Search Console.

> [!CAUTION]
> **Caution**
>
> : Check your firewall configuration or denial of service (DoS) protection. Make
...
You can help make your site move go faster if you lower your site DNS records' TTL value,
 ...
 by ISPs based on the specified [Time to Live (TTL) setting](https://wikipedia.org/wiki/Time_to_live#DNS_records).
...
If you're using the [HTML file method](https://support.google.com/webmasters/answer/9008080#html_verification) to verify ownership of your site in Search Console, make sure you don't forget to include your
...
1. **Remove any temporary blocks to crawling** . While building the new copy of a site, some
 ...
 to start the move.
2. **Update the DNS settings** . You start the move by updating the DNS records to point to
