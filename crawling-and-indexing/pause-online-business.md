...
- **Display a banner or popup** : A banner or popup div on all pages including the
 ...
 on popups and banners](https://developers.google.com/search/docs/appearance/avoid-intrusive-interstitials) .
- **Update your structured data** : If your site uses structured data (for example, [`Product`](https://developers.google.com/search/docs/appearance/structured-data/product) , [`Book`](https://developers.google.com/search/docs/appearance/structured-data/book) , [`Event`](https://developers.google.com/search/docs/appearance/structured-data/event) ), make sure to adjust it appropriately
 ...
 structured data](https://developers.google.com/search/docs/appearance/structured-data/local-business) to reflect current opening hours.
...
- **Tell Google about your updates** : To ask Google to recrawl a
 limited number of pages (for example, the home page), use [Search Console](https://developers.google.com/search/docs/crawling-indexing/ask-google-to-recrawl) . For a larger number of pages (for example, all of your product
 pages), [use sitemaps](https://developers.google.com/search/docs/guides/submit-URLs) .
...
You may decide to disable the whole website. This is an extreme measure that should only be
 taken for a very short period of time (a few days at most), as it will otherwise have **significant effects** on the website in Search, even when implemented properly.
...
- If you need to urgently disable the site for 1-2 days, then return an informational
 error page with a [`503` HTTP response status code](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/503) instead of all content. Make sure to follow the best practices for disabling a site .
...
- Confirm a `503` HTTP response status code locally by using [curl](https://curl.haxx.se/docs/manpage.html) or a similar tool. For example: curl -I -X GET "https://www.example.com/"
...
- Don't disallow all crawling in the robots.txt file. Returning a valid robots.txt file that
 ...
 Search.
- Don't block the website by returning `403` , `404` , `410` HTTP status codes, or with a `noindex` robots `meta` tag or x-robots-tag HTTP header. This will remove the
...
What if I only close the site for a few weeks? Completely closing a site even for just a few weeks can have negative consequences on
...
What if I want to exclude all non-essential products? That's fine. Make sure that people can't buy the non-essential products by limiting the site functionality.

Can I ask Google to crawl less while my site is temporarily closed? Yes, you can [reduce the Googlebot
 crawl rate](https://developers.google.com/search/docs/guides/reduce-crawl-rate), though it's not recommended for most cases. This may have some impact
...
How do I get a page indexed or updated quickly? To ask Google to recrawl a limited number of pages (for example, the home page), use [Search Console](https://developers.google.com/search/docs/crawling-indexing/ask-google-to-recrawl).
 For a larger number of pages (for example, all of your product pages), [use sitemaps](https://developers.google.com/search/docs/guides/submit-URLs).

What if I block a specific region from accessing my site? Google generally crawls from the US. If you block the US, Google Search won't be able
...
Should I use the [Removals Tool](https://support.google.com/webmasters/answer/9689846) to remove out-of-stock products? No. If you do this, customers won't be able to find first-hand information about your
